    ...
    /*
        public DialogueEvent[] EventParser(Dialogue[] dialogueArr)
        {
            ...
            for (int i = 1; i < data.Length;)
            {
                ...
                // Dialogue.cs파일의 class DialogueEvent 형식으로 각각 저장
                DialogueEvent dialogueEvent = new DialogueEvent();
                dialogueEvent.eventName = row[0]; // 발생할 이벤트의 이름
                Debug.Log(row[0]);
                // do - while 문 : 최초 한차례 무조건 실행시킨 후에 조건을 판별하고 이어서 진행할지 여부를 판단
                // csv 파일을 보면 이벤트의 끝에 "end"가 존재하므로
                // 이벤트의 이름이 존재하는 시점부터 end문이 나올때까지의 vector2값을 저장하고
                // 알맞은 Dialogue[] dialogueArr를 dialogueEvent.dialogues 에 저장
                // 그것을 판단하기 위해서 do - while문을 사용
                do
                {
                    int startNum = i - 1;
                ...
                // do의 if문에서 다음줄로 넘어간 곳의 이벤트의 이름이 공란인지를 확인하고
                // 공란이면 다음 Dialogue[] dialogueArr를 한 줄을 저장
            }
